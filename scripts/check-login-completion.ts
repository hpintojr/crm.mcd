...
const importDetailPage = readFileSync(new URL("../src/app/admin/lead-imports/[batchId]/page.tsx", import.meta.url), "utf8");
...
for (const page of [importPage, importDetailPage]) {
  assert.equal(page.includes('requireRole([...IMPORT_REVIEW_ROLES])'), true);
  assert.equal(page.includes('"OWNER", "SUPER_ADMIN", "COMPLIANCE_MANAGER"'), true);
  assert.equal(page.includes("leadImportBatch.create"), false);
  assert.equal(page.includes("leadImportBatch.update"), false);
  assert.equal(page.includes("leadImportRow.update"), false);
}
assert.equal(importDetailPage.includes("auditLog.findMany"), true);
assert.equal(importDetailPage.includes("metadata: true"), false);
