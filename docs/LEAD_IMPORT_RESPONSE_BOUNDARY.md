...
The response helper does not change batch serialization, validation issues, replay-conflict results, state errors, concurrency recovery, preview behavior, submit behavior, provenance rules, or audit evidence.
...
The check is source-only. It does not call an import endpoint, create a batch, upload rows, run a preview, submit a batch, query production, or write audit evidence.
