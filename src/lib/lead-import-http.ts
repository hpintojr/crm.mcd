 ...
 *
 * These helpers do not create a route, read secrets, access the database, or
 * trigger an import. They ensure every future handler reads the same canonical
 * header names before passing the values to the HMAC verifier.
 */
...
export type LeadImportRequestHeaderInput = {
  get(name: string): string | null;
};

export type LeadImportTransportHeaders = {
...
export function readLeadImportTransportHeaders(headers: LeadImportRequestHeaderInput): LeadImportTransportHeaders {
  const parsed = leadImportRequestHeadersSchema.parse({
...
export function leadImportUnauthorizedResponse() {
  return {
    status: 401,
    code: "LEAD_IMPORT_UNAUTHORIZED",
    message: "The signed lead-import request could not be authenticated.",
  } as const;
}
