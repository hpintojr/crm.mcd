...
/**
 * Pure workflow policy for the future durable import API.
 *
 * This module has no route handler, database dependency, or side effect. It
 * centralizes the state rules so Admin review and the eventual persistence
 * layer cannot silently auto-assign, campaign, or import an unapproved row.
 */
...
export function assertLeadImportBatchTransition(from: LeadImportBatchStatus, to: LeadImportBatchStatus) {
  if (!mayTransitionLeadImportBatch(from, to)) {
    throw new Error(`Lead import batch transition ${from} → ${to} is not allowed.`);
  }
}
...
export function assertLeadImportRecordTransition(from: LeadImportRowStatus, to: LeadImportRowStatus) {
  if (!mayTransitionLeadImportRecord(from, to)) {
    throw new Error(`Lead import record transition ${from} → ${to} is not allowed.`);
  }
}
...
export function mayCreateLeadFromImport({
  batchStatus,
  recordStatus,
}: {
  batchStatus: LeadImportBatchStatus;
  recordStatus: LeadImportRowStatus;
}) {
