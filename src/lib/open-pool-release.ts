...
export async function releaseLeadToOpenPool(input: { leadId: string; reason: string }) {
  ...
  if (!["CLAIMED", "CONTACTED", "NURTURING", "DEMO_BOOKED"].includes(lead.lifecycle)) throw new Error("This lead is not eligible for an Open Pool return.");

  const now = new Date();
  await db.$transaction([
    db.lead.update({
      where: { id: lead.id },
      data: { ownerAgentId: null, lifecycle: "AVAILABLE", pool: "OPEN", openPoolReleaseAt: now, nextActionAt: null, lastActionAt: now },
    }),
