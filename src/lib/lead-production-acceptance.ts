...
export const leadProductionAcceptanceGroups: LeadProductionAcceptanceGroup[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      { id: "ghl-appointment-hardening", title: "15. Verify GHL appointment hardening", detail: "Use the controlled GHL event harness only. Confirm suppressed/DNC Leads are ignored, booked/confirmed/rescheduled events record two-way contact, cancelled/no-show events create or expedite one callback, and Closed Won is preserved.", evidence: "Record controlled harness preview, result fields, and audit evidence. Do not enable live workflow automation from this board.", href: "/admin/integrations/test-events", action: "Open GHL harness" },
      { id: "ghl-opportunity-hardening", title: "16. Verify GHL opportunity hardening", detail: "Use the controlled GHL event harness for Won/Lost events only. Confirm terminal outcomes cancel scheduled callbacks, suppressed/DNC Leads are ignored, and late Lost cannot roll back Closed Won.", evidence: "Record controlled harness preview, result fields, and callback cancellation/preservation evidence.", href: "/admin/integrations/test-events?family=opportunity", action: "Open GHL harness" },
      { id: "aging-sweep-contract", title: "17. Verify aging sweep contract", detail: "Preview the aging sweep with /api/admin/leads/aging-preview or authorized /api/cron/leads/aging?dryRun=true. Then use controlled test data only before any real sweep. Confirm expired owned Leads would return to Open Pool and 21-day stale Open Pool records would move to Shark Tank with audit actions listed in the preview.", evidence: "Record dry-run counts, wouldReturnToOpenPool, wouldPromoteToSharkTank, cutoff, and mutationPerformed:false before any live sweep.", href: "/api/admin/leads/aging-preview", action: "Preview aging" },
