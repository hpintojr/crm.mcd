...
const documents: Array<{ type: DocType; label: string; detail: string }> = [
  ...
  { type: "W9_PAYOUT", label: "W-9 / entity tax-profile acknowledgment", detail: "Tracking only. Record the agent's company/entity name on this page when applicable; do not upload tax forms, banking data, signatures, or payment details into the MiniCRM." },
...
export default async function AgentOnboardingDocumentsPage({ params }: PageProps) {
  ...
  async function recordCompanyName(formData: FormData) {
    ...
    const parsed = companySchema.parse({ agentId: formData.get("agentId"), companyName: formData.get("companyName") || undefined, note: formData.get("note") });
    const current = await db.agent.findUnique({ where: { id: parsed.agentId }, select: { id: true, companyName: true } });
