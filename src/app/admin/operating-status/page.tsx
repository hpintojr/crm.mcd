...
export default async function OperatingStatusPage() {
  ...
  const phases: Phase[] = [
    { name: "Lead MVP", gate: features.leads, status: state(features.leads), detail: "Production schema and gated application workflow are deployed. Import review, Open Pool controls, DNC, callbacks, and GHL appointment attribution are ready for acceptance testing.", next: "Complete the controlled Lead MVP acceptance test before normal agent access.", href: "/admin/leads" },
    { name: "Client Servicing Health", gate: features.servicing, status: state(features.servicing), detail: "Production schema and gated admin/agent workspaces are deployed. Service work is trigger-driven; healthy current-paying accounts are not reassigned because they are quiet.", next: "Run the controlled servicing acceptance test before normal client-account use.", href: "/admin/servicing" },
    { name: "Commission Eligibility & Ledger", gate: features.commissions, status: state(features.commissions), detail: "Gated dashboard, ledger controls, policy checks, and an isolated commission-only schema migration are prepared. No payout action exists in this phase.", next: "Validate the isolated Neon branch, approve the separate commission schema migration, then run the controlled eligibility test.", href: "/admin/commissions" },
