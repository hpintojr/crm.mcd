...
export default async function ServicingOnboardingQueuePage() {
  ...
  async function createFromWonLead(formData: FormData) {
    ...
    if (!leadId || !packageCode) throw new Error("Lead and package code are required.");
    const [lead, linkedAccounts, selectedOwner] = await Promise.all([
      db.lead.findUnique({ where: { id: leadId }, select: { id: true, company: true, lifecycle: true, dnc: true, suppressed: true, ownerAgentId: true, ghlContactId: true } }),
      db.$queryRaw<Array<{ id: string }>>(Prisma.sql`SELECT "id" FROM "ClientAccount" WHERE "leadId"=${leadId} LIMIT 1`),
      accountOwnerAgentId ? db.agent.findFirst({ where: { id: accountOwnerAgentId, status: "ACTIVE" }, select: { id: true } }) : Promise.resolve(null),
    ]);
    if (!lead) throw new Error("Source Lead not found.");
    if (lead.lifecycle !== "CLOSED_WON" || lead.dnc || lead.suppressed) throw new Error("Only an active verified Closed Won Lead may enter client onboarding.");
    if (linkedAccounts[0]) throw new Error("This Lead already has a Client Account.");
    if (accountOwnerAgentId && !selectedOwner) throw new Error("Choose an active servicing owner or leave the account for House review.");
    await createClientAccount({
      clientName: lead.company,
      packageCode,
      leadId: lead.id,
      ghlContactId: lead.ghlContactId || undefined,
      originatingAgentId: lead.ownerAgentId || undefined,
      accountOwnerAgentId: accountOwnerAgentId || undefined,
    });
