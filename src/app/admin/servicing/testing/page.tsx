...
const steps: Step[] = [
  { id: "controlled-test-setup", title: "1. Confirm controlled test setup", detail: "Use test Client Accounts and an approved test owner. Keep Commission and Finance disabled; this test validates service handling only.", href: "/admin/operating-status", action: "Review gate state" },
  { id: "client-account", title: "2. Create a test Client Account", detail: "Use a Closed Won test Lead in the onboarding queue. Confirm one Client Account is created and retains the Lead, originating owner, and GHL identity context.", href: "/admin/servicing/onboarding", action: "Open onboarding" },
  { id: "launch", title: "3. Document launch", detail: "Complete the launch checklist. Confirm the account becomes Active when current on payments, or Payment Failed when a payment issue exists. No commission or payout should be created.", href: "/admin/servicing/launches", action: "Open launch confirmations" },
  { id: "healthy-account-protection", title: "4. Validate healthy-account protection", detail: "Keep a current-paying test Client Account quiet. Confirm that lack of routine activity alone does not create a case, reassign the account, or alter service ownership.", href: "/admin/servicing", action: "Open servicing workspace" },
  { id: "triggered-service-work", title: "5. Validate triggered service work", detail: "Create documented work for a real trigger: client request, support issue, payment problem, renewal event, escalation, or documented review. Confirm a Service Case appears with the expected priority and due time.", href: "/admin/servicing/cases", action: "Open service cases" },
  { id: "case-resolution", title: "6. Validate case resolution", detail: "Record the action and resolution. Confirm the case history remains available and the Client Account health reflects the documented result.", href: "/admin/servicing/cases", action: "Open case queue" },
  { id: "house-handling", title: "7. Validate reassignment / House handling", detail: "Use only a test account. Confirm a documented transfer preserves history and that the account moves to House only through an authorized servicing decision—not because a healthy account is quiet.", href: "/admin/servicing", action: "Open servicing workspace" },
  { id: "hard-boundaries", title: "8. Confirm hard boundaries", detail: "Verify this workflow does not create a commission, approve a payout, collect a payment, store bank data, or invoke Finance execution.", href: "/admin/finance", action: "Open Finance readiness" },
  { id: "owner-decision", title: "9. Record owner decision", detail: "Keep normal servicing access gated until every test passes or any exception has an owner-approved remediation plan.", href: "/admin/audit", action: "Open audit history" },
];
...
export default async function ServicingAcceptanceTestingPage() {
  ...
  const records = await db.auditLog.findMany({ where: { actionType: "SERVICING_ACCEPTANCE_RECORDED", entityType: "ServicingAcceptanceStep" }, orderBy: { createdAt: "desc" }, take: 250 });
