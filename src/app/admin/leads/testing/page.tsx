...
const steps: Step[] = [
  { id: "controlled-test-setup", title: "1. Confirm controlled PR #34 test setup", detail: "Use internal test agents and test records only. Confirm one certified agent can work Leads and one non-certified agent remains blocked. Keep PR #34 draft until this board records the current acceptance result.", href: "/admin/agents", action: "Review agent controls" },
  ...
  { id: "call-start-no-lock", title: "4. Verify call start is activity only", detail: "Use a Cold Lead to log call started. Confirm the action writes activity but does not set ownerAgentId, claimedAt, openPoolReleaseAt, or any soft-lock/reservation field.", href: "/portal/leads", action: "Log call start" },
  { id: "no-answer-boundary", title: "5. Verify no-answer and voicemail stay unowned", detail: "Record No Answer and Voicemail outcomes on test Cold Leads. Confirm the Lead remains unowned and not claimable from that outcome alone.", href: "/portal/leads", action: "Record no-answer" },
  { id: "two-way-contact-claim-gate", title: "6. Verify two-way-contact claim gate", detail: "Record callback-requested, qualified, or follow-up on a Cold Lead. Confirm twoWayContactAt is recorded, the Lead becomes claim eligible, and claim is still not automatic.", href: "/portal/leads", action: "Test claim gate" },
  { id: "claim-responsibility-timer", title: "7. Verify claim starts 45-day timer", detail: "Claim an eligible Lead only after two-way contact. Confirm ownerAgentId, claimedAt, lifecycle CLAIMED, and openPoolReleaseAt about 45 days after claim are set with audit/activity evidence.", href: "/portal/leads", action: "Claim eligible Lead" },
  { id: "dnc-blackout", title: "8. Verify DNC absolute blackout", detail: "Apply DNC from unowned Cold Lead flow and owned Lead flow. Confirm callbacks cancel, the record is suppressed, and the Lead disappears from sales workflows.", href: "/portal/leads", action: "Test DNC" },
  { id: "my-workspace-dashboard", title: "9. Verify My Workspace dashboard", detail: "Open /portal/workspace without leadId. Confirm it shows assigned records, callback queue, claim access, recent activity, DNC rule reminder, and claim-timer responsibility instead of returning not found.", href: "/portal/workspace", action: "Open My Workspace" },
  { id: "aging-sweep-contract", title: "10. Verify aging sweep contract", detail: "Verify the secured aging route requires Authorization, then use controlled test data to confirm expired owned Leads return to Open Pool and 21-day stale Open Pool records move to Shark Tank with audit evidence.", href: "/api/cron/leads/aging", action: "Check cron route" },
  { id: "ghl-relay-gates", title: "11. Keep GHL relay gates explicit", detail: "GHL appointment, opportunity, and inbound reply relays remain behind controlled external configuration and acceptance testing. Confirm no relay test silently changes suppressed records or bypasses owner attribution.", href: "/admin/integrations", action: "Open integrations" },
  { id: "owner-decision", title: "12. Record owner decision", detail: "Do not merge PR #34 or approve normal Lead use until every required step has a current Pass result or an approved remediation plan. Audit results do not enable feature flags automatically.", href: "/admin/audit", action: "Open audit history" },
];
  ...
  async function recordAcceptance(formData: FormData) {
    ...
    await db.auditLog.create({
      data: {
        ...
        entityId: step.id,
        reason: note,
        metadata: { module: "LEADS", phase: "PR_34_LEAD_FLOW_ALIGNMENT", outcome, stepId: step.id, stepTitle: step.title },
      },
    });
