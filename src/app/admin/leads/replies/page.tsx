...
export default async function WarmReplyQueuePage() {
  ...
  async function assignWarmReply(formData: FormData) {
    ...
    if (!lead.twoWayContactAt) throw new Error("Warm-reply assignment requires a recorded two-way contact.");
    ...
    const nextLifecycle = lead.lifecycle === "DEMO_BOOKED" ? "DEMO_BOOKED" : "CLAIMED";
    await db.$transaction(async (tx) => {
      const assigned = await tx.lead.updateMany({
        where: { id: lead.id, ownerAgentId: null, dnc: false, suppressed: false, twoWayContactAt: { not: null }, lifecycle: { in: ["AVAILABLE", "CLAIMED", "CONTACTED", "NURTURING", "DEMO_BOOKED"] } },
        data: { ownerAgentId: agent.id, lifecycle: nextLifecycle, pool: lead.pool === "COLD" ? "HOT" : lead.pool, claimedAt: lead.claimedAt ?? now, openPoolReleaseAt: releaseAt, lastActionAt: now, nextActionAt: now },
      });
      ...
      await tx.leadActivity.create({ data: { leadId: lead.id, agentId: agent.id, type: "REASSIGNED", metadata: { source: "WARM_REPLY_TRIAGE", note: parsed.note, releaseAt: releaseAt.toISOString(), rule: "TWO_WAY_CONTACT_REQUIRED" } } });
      await tx.leadCallback.create({ data: { leadId: lead.id, agentId: agent.id, dueAt: now } });
      await tx.auditLog.create({ data: { actorUserId: actor.id, actorRole: actor.role, actionType: "LEAD_WARM_REPLY_ASSIGNED", entityType: "Lead", entityId: lead.id, reason: parsed.note, metadata: { assignedAgentId: agent.id, releaseAt: releaseAt.toISOString(), priorPool: lead.pool, nextPool: lead.pool === "COLD" ? "HOT" : lead.pool, priorLifecycle: lead.lifecycle, nextLifecycle } } });
    });
