...
export default async function AdminLeadDetailPage({ params }: { params: Promise<{ leadId: string }> }) {
  ...
  async function recordCloseWon(formData: FormData) {
    ...
    if (note.length < 8) {
      throw new Error("Document the verified win before moving a Lead to Closed Won.");
    }
    ...
    if (current.lifecycle !== "DEMO_BOOKED") {
      throw new Error("Only a demo-booked Lead can be moved to Closed Won from this control.");
    }
    ...
    await db.$transaction([
      db.lead.update({
        where: { id: current.id },
        data: { lifecycle: "CLOSED_WON", lastActionAt: now, nextActionAt: null },
      }),
      db.leadNote.create({
        data: {
          leadId: current.id,
          agentId: current.ownerAgentId,
          body: `Verified Closed Won: ${note}`,
        },
      }),
      db.auditLog.create({
        data: {
    ...
    <main className="mx-auto min-h-screen max-w-7xl px-6 py-12">
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          <Link className="rounded-lg border border-ink-700 px-3 py-2 text-sm text-gray-200" href="/admin/leads">
            Lead review
          </Link>
          {lead.lifecycle === "DEMO_BOOKED" && (
            <Link className="rounded-lg border border-brand-500 px-3 py-2 text-sm text-brand-200" href="/admin/leads/handoff">
              GHL handoff
            </Link>
          )}
