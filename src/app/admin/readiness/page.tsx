...
type ReadinessCard = { label: string; value: string | number; href: string; commandHref?: string; reportHref?: string; exportHref?: string; runbookHref?: string; detail: string };
...
const acceptanceModules: AcceptanceModule[] = [
  {
    ...
    runbookHref: "/admin/leads/acceptance-runbook",
...
export default async function ReadinessBoardPage() {
  ...
  const cards: ReadinessCard[] = [
    ...acceptanceCards.map((card): ReadinessCard => ({ label: card.label, value: `${card.passed} / ${card.totalSteps}`, href: card.href, commandHref: card.commandHref, reportHref: card.reportHref, exportHref: card.exportHref, runbookHref: card.runbookHref, detail: `${card.gateEnabled ? "Controlled test enabled" : "Staged / locked"} · ${card.failed ? `${card.failed} failed` : "No failed steps"}${card.deferred ? ` · ${card.deferred} deferred` : ""}. ${card.detail}` })),
