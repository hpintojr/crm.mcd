...
export default async function WorkspacePage({ params }: PageProps) {
  ...
  if (workspace === "schedule") {
    ...
    const [upcomingAppointments, awaitingOutcome, recentOutcomes, recentActiveChanges] = await Promise.all([
      db.appointment.findMany({ where: { ...viewerFilter, status: { in: [...MEETING_ACTIVE_STATUSES] }, OR: [{ endAt: { gte: now } }, { endAt: null, startAt: { gte: now } }] }, orderBy: { startAt: "asc" }, take: 50 }),
      db.appointment.findMany({ where: { ...viewerFilter, status: { in: [...MEETING_ACTIVE_STATUSES] }, startAt: { gte: recentCutoff }, OR: [{ endAt: { lt: now } }, { endAt: null, startAt: { lt: now } }] }, orderBy: { startAt: "desc" }, take: 20 }),
      db.appointment.findMany({ where: { ...viewerFilter, status: { in: [...RECENT_OUTCOME_STATUSES] }, updatedAt: { gte: recentCutoff } }, orderBy: { updatedAt: "desc" }, take: 20 }),
      db.appointment.findMany({ where: { ...viewerFilter, status: { in: [...MEETING_ACTIVE_STATUSES] }, updatedAt: { gte: alertCutoff } }, orderBy: { updatedAt: "desc" }, take: 20 }),
    ]);
    const missedOrCancelled = recentOutcomes.filter((appointment) => appointment.status === "NO_SHOW" || appointment.status === "CANCELLED").length;

    return <PortalFeaturePage eyebrow="Appointments" title="Schedule" description="Your upcoming booked demos appear here after GHL relays them to your workspace.">{(recentActiveChanges.length > 0 || awaitingOutcome.length > 0 || missedOrCancelled > 0) && <section className="portal-card mb-6 max-w-4xl"><div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><h2 className="portal-heading text-lg font-semibold">Schedule alerts</h2><p className="portal-copy mt-1 text-sm">Live changes relayed from GHL. Booking and attendance updates remain managed in GHL; this workspace tells you what needs attention.</p></div>{missedOrCancelled > 0 && <Link className="portal-action-link shrink-0" href="/portal/tasks">Open follow-ups</Link>}</div><div className="mt-5 grid gap-3 sm:grid-cols-3"><div className="portal-callout text-sm"><p className="portal-heading font-medium">{recentActiveChanges.length} recent booking update{recentActiveChanges.length === 1 ? "" : "s"}</p><p className="portal-copy mt-1">Scheduled, confirmed, or rescheduled in the last 24 hours.</p></div><div className={awaitingOutcome.length ? "rounded-xl border border-amber-700/70 px-4 py-3 text-sm" : "portal-callout text-sm"}><p className="portal-heading font-medium">{awaitingOutcome.length} awaiting outcome</p><p className="portal-copy mt-1">Past appointment times still need a final GHL status.</p></div><div className={missedOrCancelled ? "rounded-xl border border-amber-700/70 px-4 py-3 text-sm" : "portal-callout text-sm"}><p className="portal-heading font-medium">{missedOrCancelled} follow-up check{missedOrCancelled === 1 ? "" : "s"}</p><p className="portal-copy mt-1">Recent no-show or cancelled appointment events.</p></div></div></section>}<section className="portal-card max-w-4xl"><div className="flex flex-col justify-between gap-2 sm:flex-row sm:items-center"><div><h2 className="portal-heading text-lg font-semibold">Upcoming appointments</h2><p className="portal-copy mt-1 text-sm">Only active scheduled and confirmed appointments appear here. Booking, edits, cancellations, and guest invitations stay in GHL and the connected calendar.</p></div>{isAdmin && <span className="portal-status-good text-sm font-semibold">Company view</span>}</div>{upcomingAppointments.length === 0 ? <div className="portal-callout mt-5 text-sm"><span className="font-medium portal-heading">No upcoming appointments.</span><p className="portal-copy mt-1">Completed, cancelled, and no-show appointments move out of this view automatically while their history is retained.</p></div> : <div className="mt-5 divide-y portal-border">{upcomingAppointments.map((appointment) => <article className="flex flex-col gap-3 py-4 sm:flex-row sm:items-center sm:justify-between" key={appointment.id}><div><p className="portal-heading font-medium">{appointment.title}</p><ViewerTime startAt={appointment.startAt.toISOString()} endAt={appointment.endAt?.toISOString() ?? null} /><p className="portal-copy mt-1 text-xs">{appointment.calendarName || "Mercury Call Desk calendar"} · {title(appointment.status)}</p></div>{appointment.meetingUrl && MEETING_ACTIVE_STATUSES.has(appointment.status) && <a className="portal-action-link" href={appointment.meetingUrl} target="_blank" rel="noreferrer">Join meeting</a>}</article>)}</div>}</section>{awaitingOutcome.length > 0 && <section className="portal-card mt-6 max-w-4xl"><div><h2 className="portal-heading text-lg font-semibold">Appointments awaiting outcome</h2><p className="portal-copy mt-1 text-sm">These appointment times have passed, but GHL has not relayed Completed, No-show, or Cancelled yet. Confirm the attendance result in GHL; this list protects the record from disappearing silently.</p></div><div className="mt-5 divide-y portal-border">{awaitingOutcome.map((appointment) => <article className="py-4" key={appointment.id}><div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"><div><p className="portal-heading font-medium">{appointment.title}</p><ViewerTime startAt={appointment.startAt.toISOString()} endAt={appointment.endAt?.toISOString() ?? null} /><p className="portal-copy mt-1 text-xs">{appointment.calendarName || "Mercury Call Desk calendar"} · Awaiting final status</p></div><span className="portal-status-pending text-sm font-semibold">Needs GHL outcome</span></div></article>)}</div></section>}{recentOutcomes.length > 0 && <section className="portal-card mt-6 max-w-4xl"><div><h2 className="portal-heading text-lg font-semibold">Recent appointment activity</h2><p className="portal-copy mt-1 text-sm">Completed, cancelled, and no-show appointments remain visible here for seven days. They are not deleted from Mercury Call Desk records.</p></div><div className="mt-5 divide-y portal-border">{recentOutcomes.map((appointment) => <article className="py-4" key={appointment.id}><div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"><div><p className="portal-heading font-medium">{appointment.title}</p><ViewerTime startAt={appointment.startAt.toISOString()} endAt={appointment.endAt?.toISOString() ?? null} /><p className="portal-copy mt-1 text-xs">{appointment.calendarName || "Mercury Call Desk calendar"} · {title(appointment.status)}</p></div><span className={appointment.status === "NO_SHOW" ? "portal-status-pending text-sm font-semibold" : "portal-copy text-sm"}>{title(appointment.status)}</span></div><p className="portal-copy mt-2 text-sm">{outcomeMessage(appointment.status)}</p></article>)}</div></section>}</PortalFeaturePage>;
  }

  if (workspace === "training") {
    const certification = agent?.certifications[0];
    const completedDocuments = agent?.onboardingDocs.filter((document) => document.status === "COMPLETED").length ?? 0;
    const documentsReady = completedDocuments === 4;
    return <PortalFeaturePage eyebrow="Readiness" title="Training" description="Track your path from completed onboarding to manager-approved lead access."><section className="grid max-w-4xl gap-5 md:grid-cols-3"><div className="portal-card"><p className="portal-subheading text-xs font-medium uppercase tracking-wide">Onboarding documents</p><p className={`mt-2 text-xl font-semibold ${documentsReady ? "portal-status-good" : "portal-status-pending"}`}>{completedDocuments} / 4 complete</p><p className="portal-copy mt-2 text-sm">Your manager records completion after the approved signing process. Contact your manager if a status needs correction.</p></div><div className="portal-card"><p className="portal-subheading text-xs font-medium uppercase tracking-wide">Certification</p><p className="portal-heading mt-2 text-xl font-semibold">{certification ? title(certification.decision) : "Pending manager review"}</p><p className="portal-copy mt-2 text-sm">Complete required training and any manager-requested practice before certification.</p></div><div className="portal-card"><p className="portal-subheading text-xs font-medium uppercase tracking-wide">Lead access</p><p className={`mt-2 text-xl font-semibold ${agent?.canClaimLeads ? "portal-status-good" : "portal-status-pending"}`}>{agent?.canClaimLeads ? "Eligible when enabled" : "Locked"}</p><p className="portal-copy mt-2 text-sm">Manager certification controls eligibility; the Lead module remains separately controlled by Mercury Call Desk.</p></div></section><section className="portal-card max-w-4xl"><h2 className="portal-heading text-lg font-semibold">Training library</h2><p className="portal-copy mt-3 text-sm">Product training, scripts, demo expectations, and role-play material will be published here as the program is finalized.</p></section></PortalFeaturePage>;
  }
