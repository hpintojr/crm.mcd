...
function withMode(href: string, agentFriendly: boolean) {
  return agentFriendly ? `${href}${href.includes("?") ? "&" : "?"}mode=agent` : href;
}

function actionClass(agentFriendly: boolean) {
  return agentFriendly
    ? "rounded-xl border border-brand-500 px-4 py-3 text-center text-sm font-semibold text-brand-200"
...
export default async function LeadsPage({ searchParams }: LeadsPageProps) {
  ...
  return (
    <PortalFeaturePage eyebrow="Pipeline" title="Leads" description="Work Cold Leads first. Activity does not create ownership; only verified two-way contact unlocks claiming.">
      <section className="portal-card mb-6 flex flex-wrap items-center justify-between gap-4" data-agent-surface="lead-workspace-mode">
        <div>
          <h2 className="portal-heading font-semibold">Agent-friendly mode</h2>
          <p className="portal-copy mt-1 text-sm">OpenCRM-inspired layout with larger click targets, visible row actions, and semantic labels for operator or AI-assisted workflows. Business rules are unchanged.</p>
        </div>
        <Link className="rounded-xl border border-brand-500 px-4 py-3 text-sm font-semibold text-brand-200" href={agentFriendly ? "/portal/leads" : "/portal/leads?mode=agent"} data-agent-action="toggle-agent-friendly-mode">
          {agentFriendly ? "Standard mode" : "Agent-friendly mode"}
        </Link>
      </section>
      ...
      <section className={`grid gap-6 ${agentFriendly ? "xl:grid-cols-[1.15fr_0.85fr]" : "xl:grid-cols-2"}`}>
        <section className="portal-card p-0" data-agent-surface="cold-lead-list">
          ...
          {coldLeads.length === 0 ? <p className="portal-copy px-6 py-10 text-sm">No Cold Leads are currently available.</p> : <div>{coldLeads.map((lead) => <article className={`border-b px-6 transition portal-border ${agentFriendly ? "py-5" : "py-4"} ${selectedColdLead?.id === lead.id ? "bg-black/5" : ""}`} data-agent-row="cold-lead" data-lead-id={lead.id} key={lead.id}><div className={`flex gap-4 ${agentFriendly ? "flex-col 2xl:flex-row 2xl:items-center 2xl:justify-between" : "flex-col"}`}><div><Link className={`portal-heading font-medium hover:underline ${agentFriendly ? "block py-2 text-lg" : ""}`} href={withMode(`/portal/leads?selectedCold=${lead.id}#cold-lead-review`, agentFriendly)} data-agent-action="open-cold-lead">{lead.company}</Link><p className="portal-copy mt-1 text-sm">{lead.industry || "Industry pending"} · {[lead.city, lead.state].filter(Boolean).join(", ") || "Location pending"}</p><p className="portal-copy mt-1 text-xs">{label(lead.pool)} · {label(lead.lifecycle)} · Score {lead.score}</p></div><div className={`grid gap-2 ${agentFriendly ? "sm:grid-cols-2" : "sm:grid-cols-[auto_auto]"}`}><Link className={actionClass(agentFriendly)} href={withMode(`/portal/leads?selectedCold=${lead.id}#cold-lead-review`, agentFriendly)} data-agent-action="review-cold-lead" aria-label={`Review ${lead.company}`}>Review</Link><Link className={agentFriendly ? "rounded-xl border px-4 py-3 text-center text-sm font-semibold portal-border" : "rounded-lg border px-3 py-2 text-center text-xs portal-border"} href={withMode(`/portal/leads?selectedCold=${lead.id}#cold-lead-review`, agentFriendly)} data-agent-action="record-cold-disposition" aria-label={`Record disposition for ${lead.company}`}>Disposition</Link></div></div></article>)}</div>}
        </section>
        <section className="portal-card p-0" data-agent-surface="owned-lead-list">
          ...
          {mine.length === 0 ? <p className="portal-copy px-6 py-10 text-sm">No records are assigned to you.</p> : <div>{mine.map((lead) => <article className={`border-b px-6 transition portal-border ${agentFriendly ? "py-5" : "py-4"} ${selectedLead?.id === lead.id ? "bg-black/5" : ""}`} data-agent-row="owned-lead" data-lead-id={lead.id} key={lead.id}><div className={`flex gap-4 ${agentFriendly ? "flex-col 2xl:flex-row 2xl:items-center 2xl:justify-between" : "flex-col"}`}><div><Link className={`portal-heading font-medium hover:underline ${agentFriendly ? "block py-2 text-lg" : ""}`} href={withMode(`/portal/leads?selected=${lead.id}`, agentFriendly)} data-agent-action="open-owned-lead">{lead.company}</Link><p className="portal-copy mt-1 text-sm">{lead.businessPhone} · {label(lead.lifecycle)}</p><p className="portal-copy mt-1 text-xs">Next action: {lead.nextActionAt ? pacific(lead.nextActionAt) : "Not scheduled"}</p></div><div className={`grid gap-2 ${agentFriendly ? "sm:grid-cols-2" : "sm:grid-cols-[auto_auto]"}`}><Link className={actionClass(agentFriendly)} href={withMode(`/portal/leads?selected=${lead.id}`, agentFriendly)} data-agent-action="review-owned-lead" aria-label={`Review ${lead.company}`}>Review</Link><Link className={agentFriendly ? "rounded-xl border px-4 py-3 text-center text-sm font-semibold portal-border" : "rounded-lg border px-3 py-2 text-center text-xs portal-border"} href={withMode(`/portal/leads?selected=${lead.id}#disposition`, agentFriendly)} data-agent-action="record-owned-outcome" aria-label={`Record outcome for ${lead.company}`}>Outcome</Link></div></div></article>)}</div>}
        </section>
      </section>

      {selectedColdLead && <section id="cold-lead-review" className="mt-6 grid gap-6 scroll-mt-6 xl:grid-cols-[1.15fr_0.85fr]" data-agent-surface="selected-cold-lead">
      ...
      {selectedLead && <section className="mt-6 grid gap-6 xl:grid-cols-[1.15fr_0.85fr]" data-agent-surface="selected-owned-lead">
        <section className="portal-card">
